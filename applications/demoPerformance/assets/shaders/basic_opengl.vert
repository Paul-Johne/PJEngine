...
void main() {
	...
	vec4 animationPos_weighted	= boneAttrib[1] == 0 ? vec4(pos, 1.0f)	: vec4(pos, 1.0f);	//vec4(0.0f);
	vertexData.normal			= boneAttrib[1] == 0 ? vec3(normal)		: vec3(normal);		//vec3(0.0f);
...
//			vec3(normal);
	...
	// ANTI ERROR BLOCK - Substitution for "crash variables" //
//	gl_Position			= matrices.mvp * vec4(pos, 1.0f);
//	vertexData.normal	= (matrices.viewMatrix * transpose(inverse(matrices.modelMatrix)) * vec4(normal, 0.0f)).xyz;;
	/////////////////////
}